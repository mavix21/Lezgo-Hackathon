    ...
    pub async fn create_program(
        ...
        let create_program_future =
            ProgramGenerator::create_program_with_gas_for_reply::<InitConfig>(
                self.code_id,
                InitConfig {
                    collection: init_config.collection.clone(),
            ...
            .map_err(|e| FactoryError::ProgramInitializationCustomError1(e.to_string()))?;

        let (address, _) = create_program_future
            .await
            .map_err(|e| FactoryError::ProgramInitializationCustomError2(e.to_string()))?;
